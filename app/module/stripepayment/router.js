...
router.get('/initializePaymentIntent', [
  ...
  async function initializeStripePaymentIntent (req, res) {
    ...
    if(itemDetail == null || locationDetail == null){
      ...
      if(itemDetail == null){
        errorsList.push('Error getting item detail, Please make sure item id is valid!')
      }

      if(locationDetail == null){
        errorsList.push('Error getting location detail, Please make sure location id is valid!')
      }

      new jsonFormatter().respondWithError(res,errorsList,new jsonFormatter().CODE_BAD_REQUEST)
    }else{
